...
[dnl 
dnl Get the cflags and libraries from the imlibtool script
dnl
...
#		    , enable_imlibtest=yes)
  ...
  if test x$imlib_prefix != x ; then
     ...
     if test x${IMLIBCONF+set} != xset ; then
        IMLIBCONF=$imlibtool_prefix/bin/imlib-config
     fi
  fi

  AC_PATH_PROG(IMLIBCONF, imlib-config, no)
  min_imlib_version=ifelse([$1], ,1.8,$1)
  ...
  if test "$IMLIBCONF" = "no" ; then
    ...
    imlibtool_major_version=`$IMLIBCONF $imlib_args --version | \
           sed 's/\([[0-9]]*\).\([[0-9]]*\).\([[0-9]]*\)/\1/'`
    imlibtool_minor_version=`$IMLIBCONF $imlib_args --version | \
           sed 's/\([[0-9]]*\).\([[0-9]]*\).\([[0-9]]*\)/\2/'`
...
dnl checks the results of imlibtool to some extent
dnl
      rm -f conf.imlibtest
      AC_TRY_RUN([
#include <stdio.h>
...
int main ()
{
  ...
  if (sscanf(tmp_version, "%d.%d", &major, &minor) != 2) {
    ...
    {
      printf("\n*** 'imlib-config --version' returned %d.%d, but the minimum version\n", $imlibtool_major_version, $imlibtool_minor_version);
