                       ...
                       EHTMLEditorWebExtension *extension)
{
	...
	if (delete_key || backspace_key) {
		if (key_press_event_process_delete_or_backspace_key (document, extension, key_code, control_key, delete_key))
			webkit_dom_event_prevent_default (WEBKIT_DOM_EVENT (event));
...
/* FIXME WK2
	...
	if (citation_level > 0) {
		...
		current_block = WEBKIT_DOM_NODE (dom_wrap_paragraph_length (
			document, extension, WEBKIT_DOM_ELEMENT (current_block), length));
		dom_quote_plain_text_element_after_wrapping (
                                       ...
                                       EHTMLEditorWebExtension *extension)
{
	...
	if (!e_html_editor_web_extension_get_html_mode (extension) && dom_selection_is_collapsed (document) &&
	    remove_empty_bulleted_list_item (document, extension))
		return TRUE;
