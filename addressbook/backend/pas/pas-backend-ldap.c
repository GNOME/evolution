...
search_for_dn_with_objectclasses (PASBackendLDAP *bl, const char *dn, GList **existing_objectclasses)
{
	...
	if (ldap_search_s (ldap,
		...
		while (NULL != e) {
			char *entry_dn = ldap_get_dn (ldap, e);
			int eq;

			eq = !strcmp (entry_dn, dn);
...
create_card_handler (PASBackend *backend, LDAPOp *op)
{
	...
	if (ldap_error == LDAP_SUCCESS) {
		...
		GList *l;
		char *new_vcard = e_card_simple_get_vcard_assume_utf8 (new_card);
	...
	if (current_card) {
		...
		if (mod_array->len > 0) {
			...
			if (ldap_error == LDAP_SUCCESS) {
				...
				old_vcard = e_card_simple_get_vcard_assume_utf8 (current_card);
				for (l = bl->priv->book_views; l; l = l->next) {
					CORBA_Environment ev;
...
#else
		char *ldap_dn = ldap_get_dn(ldap, e);
					    ...
					    PASRequest *req)
{
	...
	if (!strcmp (req->auth_method, "ldap/simple-email")) {
		...
		if (ldap_error == LDAP_SUCCESS) {
			...
			ldap_dn = ldap_get_dn (bl->priv->ldap, e);
			dn = g_strdup(ldap_dn);
			ldap_memfree (ldap_dn);
			ldap_msgfree (res);
		}
