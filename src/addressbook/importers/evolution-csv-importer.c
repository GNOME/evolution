...
#define E_CONTACT_ADDITIONAL_NAME 2000
#define E_CONTACT_SUFFIXES_NAME   2001
#define E_CONTACT_PREFIXES_NAME   2002
              ...
              gchar *val)
{
	...
	if (field_text)
		g_string_append_c (g_string_append (new_text, field_text), ':');
...
date_from_string (const gchar *str)
{
	...
	if (strlen (str) == 10 && str[4] == '-') { /* YYYY-MM-DD */
		date->year = str[0] * 1000 + str[1] * 100 + str[2] * 10 +
			str[3] - '0' * 1111;
			...
			if (contact_field != NOMAP) {
				if (importer == OUTLOOK_IMPORTER || importer == MOZILLA_IMPORTER) {
					...
					case E_CONTACT_ADDITIONAL_NAME: {
						EContactName *n = e_contact_get (contact, E_CONTACT_NAME);
						g_free (n->additional);
						n->additional = value->str;
						e_contact_set (contact, E_CONTACT_NAME, n);
						n->additional = NULL;
