...
unread_notify_status (EMEventTargetFolderUnread *t)
{
	...
	lookup = g_hash_table_lookup (unread_messages_by_folder, t->folder_name);
	...
	if (t->unread != old_unread) {
		if (t->unread) {
			g_hash_table_insert (unread_messages_by_folder, g_strdup (t->folder_name), GUINT_TO_POINTER (t->unread));
		} else {
			g_hash_table_remove (unread_messages_by_folder, t->folder_name);
		}
	}
	...
	if (t->is_inbox) {
		...
		unity_launcher_entry_set_count (entry, t->unread);
		unity_launcher_entry_set_count_visible (entry, t->unread != 0);
	}
#endif
}
