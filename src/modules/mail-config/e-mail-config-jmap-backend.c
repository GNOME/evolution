...
struct _EMailConfigJmapBackend {
...
G_DEFINE_DYNAMIC_TYPE (EMailConfigJmapBackend, e_mail_config_jmap_backend, E_TYPE_MAIL_CONFIG_SERVICE_BACKEND)
                             ...
                             gpointer user_data)
{
	...
	if (e_source_lookup_password_finish (E_SOURCE (source), result, &password, NULL)) {
		if (password && *password && E_IS_MAIL_CONFIG_JMAP_BACKEND (user_data)) {
			EMailConfigJmapBackend *jmap_backend = user_data;

			gtk_widget_show (jmap_backend->forget_password_button);
		}
                             ...
                             gpointer user_data)
{
	...
	if (e_source_delete_password_finish (E_SOURCE (source), result, &error)) {
		if (E_IS_MAIL_CONFIG_JMAP_BACKEND (user_data)) {
			EMailConfigJmapBackend *jmap_backend = user_data;

			gtk_widget_set_sensitive (jmap_backend->forget_password_button, FALSE);
		}
...
static void
jmap_backend_forget_password_cb (GtkWidget *button,
                                 EMailConfigJmapBackend *jmap_backend)
{
	ESource *source;

	g_return_if_fail (E_IS_MAIL_CONFIG_JMAP_BACKEND (jmap_backend));

	source = e_mail_config_service_backend_get_source (E_MAIL_CONFIG_SERVICE_BACKEND (jmap_backend));

	e_source_delete_password (source, jmap_backend->cancellable, source_delete_password_done, jmap_backend);
}

static void
mail_config_jmap_backend_insert_widgets (EMailConfigServiceBackend *backend,
                                         GtkBox *parent)
{
	EMailConfigJmapBackend *self = E_MAIL_CONFIG_JMAP_BACKEND (backend);
	...
	self->host_entry = widget;  /* do not reference */
	gtk_widget_show (widget);
	...
	self->port_entry = widget;  /* do not reference */
	gtk_widget_show (widget);
	...
	self->port_error_image = widget;  /* do not reference */
	...
	self->user_entry = widget;  /* do not reference */
	gtk_widget_show (widget);
	...
	self->forget_password_button = widget;  /* do not reference */
	...
	gtk_widget_set_margin_start (widget, 12);
	gtk_box_pack_start (GTK_BOX (parent), widget, FALSE, FALSE, 0);
	self->auth_check = widget;  /* do not reference */
	gtk_widget_show (widget);

	e_mail_config_provider_add_widgets (provider, settings, parent, FALSE);

	g_signal_connect (self->forget_password_button, "clicked",
		G_CALLBACK (jmap_backend_forget_password_cb), backend);
	...
	e_binding_bind_property_full (
		...
		self->security_combo_box, "active-id",
		G_BINDING_BIDIRECTIONAL |
		G_BINDING_SYNC_CREATE,
	...
	e_binding_bind_property (
		settings, "security-method",
		self->port_entry, "security-method",
		G_BINDING_SYNC_CREATE);
	...
	e_binding_bind_property (
		...
		self->auth_check, "active-mechanism",
		G_BINDING_BIDIRECTIONAL);

	if (port != 0)
		g_object_set (G_OBJECT (self->port_entry), "port", port, NULL);
	...
	e_source_lookup_password (source, self->cancellable, source_lookup_password_done, backend);
}

static gboolean
mail_config_jmap_backend_auto_configure (EMailConfigServiceBackend *backend,
                                         ...
                                         gboolean *out_is_complete)
{
	...
	if (page_class->provider_type == CAMEL_PROVIDER_TRANSPORT)
	...
	return e_mail_config_service_backend_auto_configure_for_kind (backend, config_lookup,
		kind, NULL, NULL, out_priority, out_is_complete);
}

static gboolean
mail_config_jmap_backend_check_complete (EMailConfigServiceBackend *backend)
{
	EMailConfigJmapBackend *self = E_MAIL_CONFIG_JMAP_BACKEND (backend);
	CamelSettings *settings;
	...
	e_util_set_entry_issue_hint (self->host_entry, correct ? NULL : _("Server address cannot be empty"));

	correct = TRUE;

	port_entry = E_PORT_ENTRY (self->port_entry);
	if (CAMEL_PROVIDER_NEEDS (provider, CAMEL_URL_PART_PORT) &&
	    !e_port_entry_is_valid (port_entry))
		correct = FALSE;

	complete = complete && correct;
	gtk_widget_set_visible (self->port_error_image, !correct);
	...
	complete = complete && correct;
	e_util_set_entry_issue_hint (self->user_entry, correct ?
...
static void
mail_config_jmap_backend_commit_changes (EMailConfigServiceBackend *backend)
{
...
static void
mail_config_jmap_backend_dispose (GObject *object)
{
	EMailConfigJmapBackend *jmap_backend = E_MAIL_CONFIG_JMAP_BACKEND (object);

	if (jmap_backend->cancellable) {
		g_cancellable_cancel (jmap_backend->cancellable);
		g_clear_object (&jmap_backend->cancellable);
	}

	/* Chain up to parent's method. */
	G_OBJECT_CLASS (e_mail_config_jmap_backend_parent_class)->dispose (object);
}

static void
e_mail_config_jmap_backend_class_init (EMailConfigJmapBackendClass *class)
{
	EMailConfigServiceBackendClass *backend_class;
	GObjectClass *object_class;

	backend_class = E_MAIL_CONFIG_SERVICE_BACKEND_CLASS (class);
	backend_class->backend_name = "jmap";
	backend_class->insert_widgets = mail_config_jmap_backend_insert_widgets;
	backend_class->auto_configure = mail_config_jmap_backend_auto_configure;
	backend_class->check_complete = mail_config_jmap_backend_check_complete;
	backend_class->commit_changes = mail_config_jmap_backend_commit_changes;

	object_class = G_OBJECT_CLASS (class);
	object_class->dispose = mail_config_jmap_backend_dispose;
}

static void
e_mail_config_jmap_backend_class_finalize (EMailConfigJmapBackendClass *class)
{
}

static void
e_mail_config_jmap_backend_init (EMailConfigJmapBackend *backend)
{
	backend->cancellable = g_cancellable_new ();
}

void
e_mail_config_jmap_backend_type_register (GTypeModule *type_module)
{
	/* XXX G_DEFINE_DYNAMIC_TYPE declares a static type registration
	 *     function, so we have to wrap it with a public function in
	 *     order to register types from a separate compilation unit. */
	e_mail_config_jmap_backend_register_type (type_module);
}
