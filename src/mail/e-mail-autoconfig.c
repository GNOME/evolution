                                   ...
                                   GCancellable *cancellable)
{
	...
	local_digest = g_alloca0 (digest_len);
	...
	for (link = records; link && !validated; link = g_list_next (link)) {
		...
		if (strv) {
			gchar *txt = g_strjoinv (NULL, strv);

			if (camel_debug ("autoconfig"))
				printf ("mail-autoconfig-pacc: DNS TXT for domain '%s': '%s'\n", domain, txt);

			validated = mail_autoconfig_pacc_check_txt_record (txt, local_digest, digest_len);
                                 ...
                                 GError **error)
{
	...
	if (!json_parser_load_from_data (parser, (const gchar *) json_data, (gssize) json_len, error)) {
	...
	protocols_obj = e_json_get_object_member (root_obj, "protocols");
	if (!protocols_obj) {
		g_set_error_literal (error, G_IO_ERROR, G_IO_ERROR_INVALID_DATA,
		                     _("PACC configuration is missing a `protocols` object"));
		g_object_unref (parser);
		return FALSE;
	}
	...
	if (!priv->imap_result.set) {
		JsonObject *imap_obj = e_json_get_object_member (protocols_obj, "imap");
		if (imap_obj) {
			const gchar *host = e_json_get_string_member (imap_obj, "host", NULL);
			if (host && *host) {
				priv->imap_result.host = g_strdup (host);
				priv->imap_result.user = g_strdup (priv->email_address);
				priv->imap_result.port = 993;
				priv->imap_result.security_method = CAMEL_NETWORK_SECURITY_METHOD_SSL_ON_ALTERNATE_PORT;
	...
	if (!priv->pop3_result.set) {
		JsonObject *pop3_obj = e_json_get_object_member (protocols_obj, "pop3");
		if (pop3_obj) {
			const gchar *host = e_json_get_string_member (pop3_obj, "host", NULL);
			if (host && *host) {
				priv->pop3_result.host = g_strdup (host);
				priv->pop3_result.user = g_strdup (priv->email_address);
				priv->pop3_result.port = 995;
				priv->pop3_result.security_method = CAMEL_NETWORK_SECURITY_METHOD_SSL_ON_ALTERNATE_PORT;
	...
	if (!priv->smtp_result.set) {
		JsonObject *smtp_obj = e_json_get_object_member (protocols_obj, "smtp");
		if (smtp_obj) {
			const gchar *host = e_json_get_string_member (smtp_obj, "host", NULL);
			if (host && *host) {
				priv->smtp_result.host = g_strdup (host);
				priv->smtp_result.user = g_strdup (priv->email_address);
				priv->smtp_result.port = 465;
				priv->smtp_result.security_method = CAMEL_NETWORK_SECURITY_METHOD_SSL_ON_ALTERNATE_PORT;
	...
	g_object_unref (parser);
	return priv->imap_result.set || priv->pop3_result.set || priv->smtp_result.set;
}
                             ...
                             GCancellable *cancellable)
{
	...
	if (!g_cancellable_is_cancelled (cancellable) &&
		...
		if (!content_type || !g_content_type_is_a (content_type, "application/json")) {
			if (camel_debug ("autoconfig"))
				printf ("mail-autoconfig-pacc: unexpected Content-Type: %s\n", content_type ? content_type : "(null)");
		} else {
			gsize data_len = 0;
			const guint8 *data = g_bytes_get_data (response_body, &data_len);
