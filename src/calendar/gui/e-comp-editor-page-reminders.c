...
ecep_reminders_selected_to_widgets (ECompEditorPageReminders *page_reminders)
{
	...
	if (duration) {
		duration_minutes = i_cal_duration_as_utc_seconds (duration) / 60;
				...
				gint *alarm_index)
{
	...
	duration = e_cal_component_alarm_trigger_get_duration (trigger);
	if (!duration || (!i_cal_duration_is_neg (duration) && i_cal_duration_as_utc_seconds (duration) != 0))
		return TRUE;

	if (i_cal_duration_get_seconds (duration) != 0)
		return TRUE;

	value = i_cal_duration_as_utc_seconds (duration) / 60;
